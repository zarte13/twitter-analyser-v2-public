...
def search_multiple(query,token, start, end):
    ...
    for j in range(len(daydates)-1):
        ...
        if path.exists("datafull.csv"):
            df1 = pd.read_csv(r'datafull.csv', index=False)
        ...
        for i in range(len(dates)-1):
            ...
            if path.exists("data4h.csv"):
                df2 = pd.read_csv(r'data4h.csv', index=False)
            ...
            rawdata = pd.read_csv(r'data.csv', index=False)
            ...
            df2.to_csv("daytweets/date-day-%s.csv"%i)
